...
- hero: 文書の冒頭に置く結論。フィールド: eyebrow（短い文脈ラベル）, title（大見出し）, lede（この文書の結論を1〜2文で。導入や前置きではありません）`;

// 部品の選び方と、文書の組み立て方は別の基準。後者を与えないと元テキストの
// 段落順がそのまま写り、見出しと段落の羅列になる
const PAGE_STRUCTURE = `ページ全体は次の順で組み立ててください。

1. 冒頭にheroを置き、ledeにこの文書の結論を書く。読み手が最初の1画面で要点をつかめるようにします。
2. 本体に入る前に、全体の関係がわかる図を1つ置く。元テキストから関係を読み取れないなら置かなくてかまいません。
3. 本体は見出しごとにまとめ、粒度の粗い話から細かい話へ進める。コードや実装の詳細はその節の後半に置きます。
4. 末尾にrecommendationを置き、判断や次にすることを書く。

元テキストの段落順に従う必要はありません。上の順序になるよう並べ替えてください。
ただしこれは型であって義務ではありません。短い文書や、結論も判断もない文書では、当てはまらない項目を省いてください。`;
